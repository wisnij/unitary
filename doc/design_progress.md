...
- *Release APK ABIs (September 23, 2026)*: Phase 10 task 5; see `openspec/changes/apk-size/`
  ...
  - The release APK now carries native code for `arm64-v8a` and `armeabi-v7a` only.  A local release build is 36,785,016 bytes (35.1 MB), against 57,146,011 bytes (54.5 MB) for the same commit with all three ABIs, so the MVP criterion of under 50 MB is met as written.  Native libraries are stored uncompressed and loaded in place (`extractNativeLibs=false`, the AGP default at `minSdk 24`), so a device keeps the whole APK, slices it cannot run included; the saving is storage on every device, not just download size
