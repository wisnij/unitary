...
Generated by `dart run tool/generate_predefined_units.dart`. Dart source with `const`
unit objects, regenerated from `units.json`. Registered into `UnitRepository` at
app startup via `registerPredefinedUnits()`.
...
dart run tool/generate_predefined_units.dart  # → updates units.json + predefined_units.dart
...
2. Run `dart run tool/generate_predefined_units.dart` to regenerate `units.json` and
   `predefined_units.dart`.
