...
Provide a shared, reusable test harness — `test/helpers/repository_overrides.dart` and `test/helpers/pump_app.dart` — for widget and provider tests that depend on Riverpod must-override providers (`settingsRepositoryProvider`, `worksheetRepositoryProvider`, `freeformHistoryRepositoryProvider`, `currencyRateRepositoryProvider`). The harness supplies default in-memory repositories and a `pumpApp` convenience so individual tests no longer hand-roll `SharedPreferences.setMockInitialValues` + repository construction + override-list boilerplate.
