...
- [x] 1.1 Update `currency_rate_repository_test.dart`: change `_xauDescriptor.originalUnit`
      to a `goldprice` stub (id `goldprice`, no aliases) matching real
      `_buildCurrencyDescriptors()` output; add a `_meter` unit stub.
- [x] 1.2 Add tests for `CurrencyRateRepository.descriptorForUnit(Unit, descriptors)`:
...
- [x] 1.3 Update `lastUpdatedForUnit` tests to pass `Unit` objects (`_euro`,
      `_goldounce`, `_meter`) instead of bare ID strings; add a case for
      "currency unit with no rates loaded yet returns null".
- [x] 1.4 Implement `CurrencyRateRepository.descriptorForUnit` (static) in
      `lib/features/currency/data/currency_rate_repository.dart`.
- [x] 1.5 Update `lastUpdatedForUnit` signature to
...
- [x] 2.1 Add tests for `formatShortDate(DateTime)` in
      `test/shared/utils/date_formatter_test.dart` (e.g. `2026-06-06` ->
      `"Jun 6, 2026"`, single-digit and double-digit days, all twelve months).
- [x] 2.2 Implement `formatShortDate` in `lib/shared/utils/date_formatter.dart`.

## 3. Unit entry detail screen

- [x] 3.1 Update `_buildScreen` test helper in
      ...
      with rates per test).
- [x] 3.2 Add tests: "Last updated" section shown for `euro` with a stored
      rate date, formatted via `formatShortDate`.
- [x] 3.3 Add test: "Last updated" section shown for `goldounce` resolving to
      the `goldprice` stored date.
- [x] 3.4 Add test: "Last updated" section shown for `goldprice` directly.
- [x] 3.5 Add test: currency unit with no rates loaded yet shows "Using
      built-in rates".
- [x] 3.6 Add test: non-currency unit (e.g. `meter`) shows no "Last updated"
      section.
- [x] 3.7 Add test: "Last updated" section appears after the "Value" section
      in widget order.
- [x] 3.8 Wire `currencyRateRepositoryProvider` into `UnitEntryDetailScreen`
...
- [x] 3.9 Implement the "Last updated" section in `_UnitDetailBody`, using
...
- [x] 4.1 Run `flutter test --reporter failures-only`.
- [x] 4.2 Run `flutter analyze`.
- [x] 4.3 Update `doc/design_progress.md` with a dated entry summarizing the
      change.
