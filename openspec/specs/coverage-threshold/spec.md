...
Defines the CI coverage gate: a checked-in checker (`tool/check_coverage.dart` +
`tool/check_coverage_lib.dart`) that parses the LCOV report from `flutter test
--coverage`, restricts it to a configured scope with exclusions, and fails the
build when line coverage falls below a configured minimum — turning the project's
">80% for parser and core domain logic" MVP success criterion from a measured
number into an enforced one.  Also defines the expected-absence allowlist that
keeps files legitimately missing from the report from either hiding a regression
or raising a false alarm, and the CI wiring that runs the gate.
