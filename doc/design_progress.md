...
- *Project site on a custom domain (September 25, 2026)*: see `openspec/changes/pages-site/`
  - 2215 tests passing, `flutter analyze` clean, coverage 95.88% over `lib/`
  ...
  - **Generator** (`tool/generate_web_docs{,_lib}.dart`, 69 tests): writes into a site directory (default `build/site`), replacing its contents, and copies in the local images the pages reference (the README's screenshots).  Relative links resolve against the source's directory in the repository: a link to a published document points at its page, any other existing file or directory goes to `github.com/wisnij/unitary/blob/main/…` (or `tree/main/…`), and a missing target fails.  This replaces the old hard failure on unconverted `.md` links, which existed to force this decision before `doc/` links appeared.  Each document now declares its own footer link (home → repository, privacy → home), the site index's title is its heading alone ("Unitary"), and the stylesheet keeps images and wide tables within the content width on phones.  The tool refuses an output directory that contains the repository, or that lies in it anywhere other than within `build/`, and renders everything before deleting anything.  One test generates against the real repository, so a bad link in the README also fails `flutter test`
