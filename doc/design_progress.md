...
- *Integration test suite, initial pass (July 26, 2026)* — code review finding F9 and the Phase 9 "Comprehensive testing" task; see `openspec/changes/integration-tests/`
  ...
  - New `test_driver/integration_test.dart` (`integrationDriver()` entrypoint) and `integration_test` added as an SDK dev dependency
  - Runs against the `chrome` target — confirmed no Android/iOS emulator is needed for any of this (`main.dart` has no platform branches)
  - Discovered mid-implementation, correcting the plan: `flutter test integration_test/ -d chrome` (the assumed invocation) doesn't exist — web targets aren't supported by `flutter test` for `integration_test` in Flutter 3.44. The actual mechanism is `flutter drive --driver=test_driver/integration_test.dart --target=<file> -d chrome`, which needs a separately-running `chromedriver` WebDriver server, and the `--web-run-headless` flag — without it, `flutter drive -d chrome` launches a **visible** browser window (`--headless` alone only governs the WebDriver-side "driver" browser, not the separate app-hosting one)
  - Local diagnosis (on a real dev desktop, not a clean CI-like box) got as far as chromedriver establishing a session and the DWDS debug service starting, then hung with an unresolved `AppConnectionException` before any test result was reported; root cause not isolated (possibly local-machine-specific networking, possibly a general web+`flutter drive`+headless fragility in this Flutter version). No passing local run was achieved
  - Mitigation: the whole suite is gated off by default in CI via `ENABLE_CHROME_INTEGRATION_TESTS: 'false'` in `.github/workflows/ci.yml`'s `test` job env (consumed by a new `if:`-guarded step in `.github/actions/test/action.yml`) — a one-line flip once a passing run is confirmed against the clean `ubuntu-latest` runner. The Dart test code itself is written and `flutter analyze`-clean; the unresolved risk is scoped to the web-driver tooling path, not test-logic correctness
  - Full existing suite unaffected: 2043 tests passing, `flutter analyze` clean (plain `flutter test` does not pick up `integration_test/`)
