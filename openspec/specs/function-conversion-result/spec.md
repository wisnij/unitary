# Capability: function-conversion-result

## Purpose

Defines the `FunctionConversionResult` evaluation result subtype, the logic in
`FreeformNotifier` that produces it when the output field names a function, and
the `ResultDisplay` rendering for that result type.
