...
- [x] 1.1 Write a test demonstrating the bug: register a dynamic currency
      ...
      1.2)
- [x] 1.2 Change `_worksheetParserProvider` in
...
- [x] 2.1 Write tests for `WorksheetNotifier`: after
...
- [x] 2.2 Extract the `build()` seeding loop (persisted sources →
...
- [x] 2.4 Confirm tests from 2.1 pass

## 3. Documentation

- [x] 3.1 Add an entry to `doc/design_progress.md`'s implementation
      progress log describing the fix and the new
      `unitRepositoryVersionProvider` listener in `WorksheetNotifier`

## 4. Verification

- [x] 4.1 Run `flutter test --reporter failures-only` and confirm all
      tests pass
- [x] 4.2 Run `flutter analyze` and confirm no issues
