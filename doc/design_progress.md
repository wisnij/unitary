...
- *Integration test suite (July 27, 2026)* — code review finding F9 and the Phase 9 "Comprehensive testing" task; see `openspec/changes/integration-tests/`
  ...
  - All 8 scenarios across the 3 files now pass repeatedly against the real emulator. CI wiring (`reactivecircus/android-emulator-runner`) is written but gated off by `ENABLE_ANDROID_INTEGRATION_TESTS: 'false'` in `.github/workflows/ci.yml` until a real CI run is observed to pass — the specific GitHub Actions configuration is the one thing not yet verified, unlike the Dart test logic itself
