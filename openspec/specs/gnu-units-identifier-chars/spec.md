...
Defines the character set rules for lexer identifier tokens, covering which characters may start an identifier, which may appear in the body, which characters terminate identifier scanning, and the error conditions for trailing periods and underscores.
