...
Provides a persistent, session-spanning record of successful freeform
conversions so users can quickly revisit past expressions without retyping
them.  Each entry captures the "Convert from" and "Convert to" field values
together with the formatted result, and is accessible via an AppBar button
that opens a modal list.
