...
- *Project site on a custom domain (September 25, 2026)*: see `openspec/changes/pages-site/`
  ...
  - **One web build with a relative base href.**  `web/index.html` hard-codes `<base href="./">` and no job passes `--base-href`.  This is sound because the app uses Flutter's default hash URL strategy, so the document path never changes.  `build-site` reuses `build-web`'s output instead of building the app a second time, so the deployed app and the release web zip are the same build.  The zip previously worked only at a server root and now works from any directory.  Verified by a spike before anything else was built: `flutter build web` accepted the placeholder-free `index.html` with no warning.  Served under `/app/`, every request resolved there with status 200, and the License and Privacy screens loaded their Markdown assets
