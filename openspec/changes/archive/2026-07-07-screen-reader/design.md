...
`WorksheetRowWidget` (`worksheet_row_widget.dart`) was orphaned — `WorksheetScreen` builds table rows inline; only the widget's own test referenced it.  This change modifies `worksheet_screen.dart`; the orphan and its test (which encoded the superseded red-text error behavior) were deleted during verification cleanup.
