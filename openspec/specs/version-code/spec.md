...
Defines how the Android version code is derived from Unitary's semantic version
(`MAJOR × 1000000 + MINOR × 1000 + PATCH`), keeping the version name as the
single source of truth with no separately maintained counter.  Also defines the
bounds the derivation enforces — components that cannot be encoded, and codes
beyond Android's accepted range — and the release tooling's obligation to record
`X.Y.Z+CODE` in `pubspec.yaml` and to verify that a recorded code still matches
the version name it is paired with.
