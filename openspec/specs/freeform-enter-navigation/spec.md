# Spec: Freeform Enter Navigation

## Purpose

Defines how the keyboard submit (Enter) action behaves in the freeform screen's
two expression fields, advancing focus from the "Convert from" field to the
"Convert to" field rather than dismissing the keyboard, while still triggering
evaluation.
