...
Nonlinear unit definitions (unit name containing `(` but not `[`) SHALL be
parsed as `defined_function` or `function_alias` entries rather than
`unsupported` entries.  All entries that were previously classified as
`"unsupported"` with `reason: "nonlinear_definition"` SHALL now be classified
as `"defined_function"` or `"function_alias"`.
...
#### Scenario: Nonlinear unit produces defined_function entry (not unsupported)
- **WHEN** the unit name contains `(` but NOT `[` (function-argument syntax,
  e.g., `circlearea(r)`)
- **THEN** `type` is `"defined_function"` with the parsed fields
- **AND** the entry is NOT placed in the `"unsupported"` section

#### Scenario: Zero-arg nonlinear alias produces function_alias entry
- **WHEN** the unit name ends with `()` and the body is a single identifier
  (e.g., `tempcelsius() tempC`)
- **THEN** `type` is `"function_alias"` and `target` is that identifier
- **AND** the entry is NOT placed in the `"unsupported"` section
...
- **THEN** prefix-namespace entries (whose source name ended with `-`) appear
  ...
  all other entries (including piecewise and defined_function) appear under
  `"units"`
...
- **AND** `definition` is included for all types except `"unsupported"`,
  `"piecewise"`, and `"defined_function"`
...
- **AND** `params`, `forward`, `inverse` (if non-null), `domainUnits`,
  `domainBounds`, `rangeUnit` (if non-null), `rangeBounds` (if non-null), and
  `noerror` are included for `"defined_function"` entries
