...
Persist the user's freeform "Convert from" and "Convert to" field text across
app sessions using SharedPreferences, and immediately re-evaluate the restored
expression on launch so the result display is populated without any user
interaction.
