...
- *Privacy policy and web-doc generation (September 6, 2026)* — Phase 10 release blocker; see `openspec/changes/privacy-policy/`
  ...
  - **The permission list was verified against the real artifact rather than assumed**, which is worth doing for a claim made in a legal document: `aapt2 dump badging` on the signed release APK (`versionCode='9007'`) reports `INTERNET` **and** `dev.wisnij.unitary.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION`.  The second is Flutter-declared, namespaced to the app, and used for its own private broadcast receivers on newer Android versions; the policy lists both and explains that neither grants access to anything about the user
  ...
  - Two testing details worth remembering, both places where the obvious assertion is the wrong one.  The rendered-H1 check is attribute-tolerant rather than exact markup, because the `gitHubWeb` extension set emits heading ids — kept deliberately, so a published document can deep-link to its own sections, and pinned by its own test.  And the `PrivacyScreen` link-tap fixture puts the link alone in its own paragraph: tapping the centre of a mixed paragraph lands on prose rather than on the link's `TapGestureRecognizer`, so a test written the obvious way passes without ever launching anything
...
- *Release APK ABIs (September 23, 2026)*: Phase 10 task 5; see `openspec/changes/archive/2026-09-23-apk-size/`
  - 2174 tests passing (19 new), `flutter analyze` clean
  - The release APK now carries native code for `arm64-v8a` and `armeabi-v7a` only.  The published v0.9.9 APK is 36,333,428 bytes (34.7 MB), so the MVP criterion of under 50 MB is met as written.  A local release build of the same configuration was 36,785,016 bytes (35.1 MB), against 57,146,011 bytes (54.5 MB) for the same commit with all three ABIs.  Native libraries are stored uncompressed and loaded in place (`extractNativeLibs=false`, the AGP default at `minSdk 24`), so a device keeps the whole APK, slices it cannot run included; the saving is storage on every device, not just download size
  - **Why keep 32-bit ARM:** the Android 16 CDD, §7.6.1 (unchanged from 14 and 15), requires a single ABI on devices under 2 GB RAM and strongly recommends 32-bit-only userspace from 2 GB to under 4 GB.  New Android Go devices therefore ship 32-bit userspace, often on 64-bit chips, and an arm64-only APK would not install on them.  Their owners can't tell from a spec sheet that advertises a 64-bit chip.  ChromeOS also documents 32-bit ARM translation on every Android-capable Chromebook, but not 64-bit translation on every x86 one
  - **Why drop x86_64:** Intel cancelled its phone chips in 2016 and the last x86 phones (Spreadtrum SC9853i, an Intel Airmont design) shipped in 2017–2018.  The one found that meets `minSdk 24`, the Leagoo T5c on Android 7.0, reports `arm64-v8a` and `armeabi-v7a` alongside `x86_64`, so it translates ARM.  Other x86_64 Android is emulators, Chromebooks, and hobbyist PC ports
  - **`--split-per-abi` was rejected for a reason worth remembering:** for split builds the Flutter Gradle plugin overrides each APK's version code with `abiIndex × 1000 + versionCode` (`armeabi-v7a` = 1, `arm64-v8a` = 2, `x86_64` = 4; `FlutterPlugin.kt`).  Under the version-code scheme that offset lands in the MINOR field, so an arm64 split of 1.0.0 would carry 1.2.0's code.  `-Pforce-version-code-ignoring-abi=true` turns it off.  A fat APK never triggers it
  - **`--target-platform` alone leaves a crash behind.**  It only controls which ABIs Flutter compiles `libflutter.so` and `libapp.so` for.  Without `--split-per-abi`, `configureAbiWithoutSplits` clears `defaultConfig.ndk.abiFilters` and refills it with all three default ABIs, so dependency libraries still ship for x86_64.  This project has one: androidx DataStore's `libdatastore_shared_counter.so`, via `shared_preferences_android`.  The result is a `lib/x86_64/` holding that library and no engine, and since the package manager picks the preferred ABI whenever the APK has any library for it, an x86_64 device would select it and crash instead of falling back to ARM under translation.  Flutter issue #192530 reports the same failure for app bundles
  - The fix is a release-only packaging exclusion in `android/app/build.gradle.kts`: `androidComponents.onVariants(selector().withBuildType("release"))` adding `**/x86_64/**` to `packaging.jniLibs.excludes`.  It runs at packaging, after Flutter's filter reset, and covers dependency libraries as well as Flutter's.  The pattern has a leading `**/` because AGP's documentation doesn't say what path `jniLibs` patterns are matched against; a build confirmed it works.  Debug and profile builds keep x86_64, so the integration suite on the x86_64 emulator is unaffected.  A static `packaging {}` block was rejected because it applies to every build type, and build-type `ndk.abiFilters` because it would compete with Flutter's reset
  - Both APK jobs pass `--target-platform android-arm,android-arm64`, which only saves compiling an engine the exclusion would discard: a build without the flag produced a byte-identical size.  Both then run `tool/verify_apk_abis.dart`, which lists the APK with `unzip -Z1` and fails unless the ABI set is exactly the two ARM ABIs, each with `libflutter.so` and `libapp.so`.  Pinning the full set rather than checking for the absence of `x86_64` also catches a partial directory for any other ABI a future dependency or Flutter upgrade might bring.  The logic is a tested `_lib` (19 tests), written test-first, rather than a shell script, since the set comparison and partial-directory detection are real logic
  - **Verified on an emulator that translates ARM**, which took finding one: the installed API 33 `google_apis` x86_64 image lists only `x86_64` and has no native bridge.  The API 35 `google_apis` x86_64 image has `ro.dalvik.vm.native.bridge=libndk_translation.so`, for 64-bit ARM only (ABI list `x86_64,arm64-v8a`).  On it the release APK installed with `primaryCpuAbi=arm64-v8a`, loaded `libflutter.so` from `lib/arm64-v8a/`, and stayed running on the Freeform screen.  The 32-bit translation path that some x86 Chromebooks depend on was not tested locally.  Note that `avdmanager` wrote the new AVD to `~/.config/.android/avd` while the emulator reads `~/.android/avd` on this machine, so launching it needs `ANDROID_AVD_HOME`
  - Carried to Phase 10 task 7: the exclusion is scoped to the release variant and will probably reach the app bundle too, which looks harmless but should be confirmed there
- *Project site on a custom domain (September 25, 2026)*: see `openspec/changes/pages-site/`
  - 2219 tests passing, `flutter analyze` clean, coverage 95.88% over `lib/`
  - The site moves to `https://unitary.wisnij.dev`, with a new layout: the rendered `README.md` at `/`, the web app at `/app/`, and the privacy policy at `/privacy`.  GitHub redirects every `wisnij.github.io/unitary/<path>` to the same path on the custom domain, so the address in every released APK's bundled policy (`…/unitary/privacy`, no slash) keeps working.  Old app bookmarks now land on the README, one link from the app
  - Timed ahead of the Play submission: no privacy-policy URL had been registered yet, so the address could still move to one the project owns.  Each generated page declares its canonical address on the custom domain, and the `pages-site` spec requires any future host to serve those extensionless paths
  - **Extensionless URLs come from the Pages server, not from Jekyll.**  Checked on the live site, which already ran without Jekyll: `/unitary/index` served `index.html` (200), while `/unitary/manifest` returned 404 even though `manifest.json` exists.  The server tries `.html` and nothing else.  So the policy is generated as `privacy.html` and served at `/privacy` with no redirect.  `/privacy/` returns 404, which is accepted: nothing the project controls links to that form
  - **Published through GitHub Actions** (`upload-pages-artifact` + `deploy-pages`) instead of force-pushing to `gh-pages`.  The custom domain lives in repository settings rather than in a `CNAME` file every push must carry, Jekyll never runs, and the deploy no longer needs `contents: write`.  `web/.nojekyll` is therefore removed.  `build-site` runs on every push and pull request, so a broken link in the README fails CI before merge; `deploy-site` runs only on `main`.  Pages are generated in CI and no longer committed, so the committed `web/privacy/index.html` and the `generate-web-docs` pre-commit hook are gone
  - Both upload actions drop dotfiles by default (`upload-pages-artifact` since v4), so both set `include-hidden-files: true`
  - **One web build with no `<base>` element.**  `web/index.html` no longer has a `<base>` element and no job passes `--base-href`, so relative URLs resolve against the directory the page was loaded from.  This is sound because the app uses Flutter's default hash URL strategy, so the document path never changes.  `build-site` reuses `build-web`'s output instead of building the app a second time, so the deployed app and the release web zip are the same build.  The zip previously worked only at a server root and now works from any directory.  A relative `<base href="./">` would behave the same in a release build, but the `flutter run` development server rejects any base href that does not start and end with `/` (`flutter_tools/lib/src/web_template.dart`); it only checks when a `<base>` element has an `href`, so leaving the element out satisfies it.  Verified before the rest was built: served under `/app/`, a release build's requests all resolved there with status 200 and the License and Privacy screens loaded their Markdown assets, and `flutter run -d web-server` started cleanly and served a working app
  - **Headless Chromium's `--screenshot` and `--dump-dom` can't check a Flutter web app**: both capture before the Flutter view mounts and show a blank page, even for the live site.  What worked was driving Chromium over the DevTools protocol from a throwaway Node script (Node 25 has a built-in `WebSocket`): wait in real time, click `flt-semantics-placeholder` to enable semantics, then send real mouse events to the centre of a semantics node found by its `aria-label`.  Calling `.click()` on the DOM node hit the wrong widget
  - **Generator** (`tool/generate_web_docs{,_lib}.dart`, 73 tests): writes into a site directory (default `build/site`), replacing its contents, and copies in the local images the pages reference (the README's screenshots).  Relative links resolve against the source's directory in the repository: a link to a published document points at its page, any other existing file or directory goes to `github.com/wisnij/unitary/blob/main/…` (or `tree/main/…`), and a missing target fails.  This replaces the old hard failure on unconverted `.md` links, which existed to force this decision before `doc/` links appeared.  Each document now declares its own footer link (home → repository, privacy → home), the site index's title is its heading alone ("Unitary"), and the stylesheet keeps images and wide tables within the content width on phones.  The tool refuses an output directory that contains the repository, or that lies in it anywhere other than within `build/site` (so `build/web` and other build output are safe), and renders everything before deleting anything.  One test generates against the real repository, so a bad link in the README also fails `flutter test`
  - About screen: "Project home" now opens `https://unitary.wisnij.dev/`, and a new "Source code" entry opens the GitHub repository, making the AGPL §13 source offer an entry of its own.  The entries now run Project home, Source code, Privacy policy, License terms, Version, Build
  - `PRIVACY.md` names the new addresses and has a new effective date, since the date is how a reader of a bundled copy tells which version they have.  The Cloudflare record for `unitary` must be **DNS-only**: a proxied record would add Cloudflare as a second party that sees and logs requests, which the policy does not disclose.  This is a spec requirement, not only a step for getting the certificate
  - Moving to a new origin means web users' browser-stored data (settings, history, worksheet values, fetched rates) does not carry over from `wisnij.github.io`.  Unavoidable with any domain change
  - The cutover is manual and happens after merge (switch the Pages source, set the custom domain, change DNS, enforce HTTPS, delete `gh-pages`).  It is listed as tasks 9.x of the change
