...
11. **Phase 10: Release** - **NEXT** — two genuine blockers carry over from Phase 9, neither of which was Phase 9 work: the Android release build is signed with the **debug** key (`android/app/build.gradle.kts` still has the `flutter create` TODO, so switching to a real key later breaks in-place upgrades for anyone who installed the first published APK), and there is no **privacy policy**.  Worth a decision alongside them: code-review finding **F8** (worksheet AppBar dropdown overflows at ≲410 dp), the only user-visible item among the deferred findings
