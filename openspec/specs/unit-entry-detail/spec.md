# Unit Entry Detail Spec

## Purpose

Define the requirements for the detail page shown when a user taps an entry in
the unit browser.  The page displays the full metadata for a unit, prefix, or
function: name, aliases, type, description, definition, value, domain/range,
and (for piecewise functions) a control-point table.
