...
**Reason**: The site now has two destinations (home page and app), and the
right footer link differs by page; the home page links to the repository.
**Migration**: Replaced by "Each page declares its own footer link".

### Requirement: Links to unconverted Markdown documents fail the build
**Reason**: The README links to many unpublished `doc/` files; failing on
them was a placeholder until a rewriting rule was chosen.
**Migration**: Replaced by "Relative links are resolved against the
repository", which rewrites such links to GitHub and fails only on missing
targets.

### Requirement: Generated pages are committed and kept in sync by a pre-commit hook
**Reason**: Pages are now generated in CI on every push and pull request, so
committing them and guarding drift with a hook is unnecessary.
**Migration**: The committed `web/privacy/index.html` and the
`generate-web-docs` hook are removed; the self-identifying marker survives as
"Generated pages identify themselves"; CI generation is specified by the
`pages-site` capability.

### Requirement: The deploy branch is published verbatim
**Reason**: The site is deployed through GitHub Actions, which never runs
Jekyll, and there is no deploy branch; `web/.nojekyll` no longer does
anything.
**Migration**: `web/.nojekyll` is removed; unmodified publication is
specified by the `pages-site` capability.
