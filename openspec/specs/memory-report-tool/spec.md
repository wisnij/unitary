...
Provide a checked-in script (`tool/memory_report.dart` + `tool/memory_report_lib.dart`) reporting the stage-by-stage memory footprint of the core-domain data structures (unit catalog, resolution cache, browse catalog, currency descriptors), so memory questions are answered by order-of-magnitude measurement.  Meaningful numbers require an AOT build (`dart compile exe`); under the JIT VM the in-process compiler dominates RSS and the tool prints a warning.
