...
Support for an `[Unreleased]` section in `CHANGELOG.md` that tracks
conventional commits since the last release tag.  The `release` tool gains a
`changelog` subcommand to (re)write the section, and the existing `release`
subcommand is updated to consume the section when present.
