Changelog
=========

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).


[Unreleased]
------------

### Added

- Add privacy policy
- Drop x86_64 image to reduce APK size

### Documentation

- Update implementation plan with release signing complete


[0.9.8] - 2026-09-05
--------------------

### Documentation

- Build out Phase 10 steps leading up to 1.0 release


[0.9.7] - 2026-09-01
--------------------

### Documentation

- Rewrite README and documentation pages
- Update documentation strings and close out Phase 9


[0.9.6] - 2026-08-05
--------------------

### Changed

- Decouple UserSettings from Flutter

### Fixed

- Hint freeform input fields properly

### Documentation

- Add code review findings
- Mark integration tests complete


[0.9.5] - 2026-07-17
--------------------

### Changed

- Reduce unnecessary freeform screen rebuilds


[0.9.4] - 2026-07-12
--------------------

### Added

- Wrap long freeform expressions


[0.9.3] - 2026-07-08
--------------------

### Added

- Add semantic labels to operator keys and completion overlay
- Add accessibility semantics for screen readers
- Ensure UI element contrast meets WCAG standards


[0.9.2] - 2026-07-03
--------------------

### Added

- Don't choose a default worksheet on first startup
- Set maximum size for UI elements on wide displays

### Fixed

- Constrain UI elements to device-specific safe areas


[0.9.1] - 2026-06-28
--------------------

### Added

- Add responsive layouts for different screen sizes

### Fixed

- Move focus to the output box after hitting Enter


[0.9.0] - 2026-06-18
--------------------

### Added

- Add application icon


[0.8.3] - 2026-06-15
--------------------

### Added

- Show last-updated date for currency units in unit browser
- Display last updated time on the currency worksheet


[0.8.2] - 2026-06-11
--------------------

### Added

- Add a currency worksheet

### Fixed

- Update stale rates on the currency worksheet


[0.8.1] - 2026-06-07
--------------------

### Fixed

- Add `INTERNET` permission to the Android app so it can fetch exchange rates


[0.8.0] - 2026-06-07
--------------------

### Added

- Fetch recent exchange rates to update currency units


[0.7.6] - 2026-06-05
--------------------

### Fixed

- Fix pipeline Flutter version to match local version


[0.7.5] - 2026-06-05
--------------------

### Added

- Add output expressions to freeform conversion examples
- Allow an open paren after function names when doing function lookups or conversions

### Fixed

- Deselect input fields after selecting an example expression


[0.7.4] - 2026-05-24
--------------------

### Added

- Add a smart completion dropdown of unit and function names

### Changed

- Move raw unit files to `assets/units/`


[0.7.3] - 2026-05-21
--------------------

### Added

- Show operator keys in freeform mode


[0.7.2] - 2026-05-15
--------------------

### Added

- Add input history dialog to freeform input mode


[0.7.1] - 2026-05-12
--------------------

### Added

- Remove persistence of freeform mode inputs between sessions


[0.7.0] - 2026-04-24
--------------------

### Added

- Persist inputs and worksheet selection between sessions

### Documentation

- Update Phase 6 in the implementation plan


[0.6.10] - 2026-04-22
---------------------

### Added

- Add angle worksheet
- Sort worksheets by name

### Fixed

- Make the scrollbar thumb on the browse page easier to use


[0.6.9] - 2026-04-19
--------------------

### Changed

- Cache unit resolution more widely


[0.6.8] - 2026-04-15
--------------------

### Added

- Show base units next to group names in the unit browser
- Add electric vehicle efficiency label

### Changed

- Add per-page `Scaffold`/`AppBar` and remember page states
- Make `EvalContext.repo` non-nullable

### Fixed

- Consistent labels for group-by buttons
- Regenerate predefined units to agree with supplementary labels


[0.6.7] - 2026-04-12
--------------------

### Added

- Add swap button between the freeform input boxes
- Add more example expressions

### Fixed

- Allow expanding and collapsing groups while searching


[0.6.6] - 2026-04-12
--------------------

### Added

- Display example expressions on the freeform page when idle
- Show fast-scrolling control on the browse page
- Collapse groups by default on the alphabetical browse page
- Show expand-all and collapse-all buttons on the browse page
- Label energy/area as energy flux
- Label inverse farads as Elastance


[0.6.5] - 2026-04-11
--------------------

### Added

- Add a Browse page for viewing unit and function definitions

### Fixed

- Handle implicit multiplication with an inverse function call


[0.6.4] - 2026-04-02
--------------------

### Added

- Support reciprocal conversions in freeform mode

### Fixed

- Allow browsing conformable units from reciprocal conversion


[0.6.3] - 2026-03-31
--------------------

### Added

- List conformable units from the freeform entry page


[0.6.2] - 2026-03-30
--------------------

### Added

- Show more specific error messages in worksheets
- Add "Torr" as an alias of the torr unit

### Fixed

- Don't display redundant unit definition lines
- Use singular labels on worksheet rows


[0.6.1] - 2026-03-28
--------------------

### Added

- Include more units in default worksheets

### Fixed

- Clarify behavior settings heading
- Display worksheet mode as a top-level page


[0.6.0] - 2026-03-27
--------------------

### Added

- Add worksheet mode and initial predefined worksheets


[0.5.12] - 2026-03-23
---------------------

### Fixed

- Require input to be an expression when converting
- Don't display redundant definitions


[0.5.11] - 2026-03-19
---------------------

### Added

- Display unit definitions


[0.5.10] - 2026-03-15
---------------------

### Added

- Add an About screen to the left-hand drawer menu
- Display function name when converting to a function


[0.5.9] - 2026-03-14
--------------------

### Added

- Handle function definition requests and conversion output

### Documentation

- Standardize header style in Openspec artifacts


[0.5.8] - 2026-03-12
--------------------

### Added

- Implement defined functions

### Changed

- Remove affine units


[0.5.7] - 2026-03-07
--------------------

### Added

- Implement inverse function operator `~`

### Fixed

- Parse piecewise linear functions with parens in the range unit


[0.5.6] - 2026-03-07
--------------------

### Fixed

- Update release workflow to fix tag fetching bug


[0.5.5] - 2026-03-07
--------------------

### Added

- Implement and import piecewise linear functions


[0.5.4] - 2026-03-04
--------------------

### Added

- Support logB(x) as a shorthand for log(x)/log(B)


[0.5.3] - 2026-03-03
--------------------

### Added

- add function types and proper builtin functions

### Changed

- rename builtin units to predefined units


[0.5.2] - 2026-03-01
--------------------

### Added

- allow more characters in identifiers

### Fixed

- improve display of 1/x dimensions

### Documentation

- free software statement


[0.5.1] - 2026-02-27
--------------------

### Added

- accept single-digit suffix as a shorthand for exponent
- raise an error when evaluating unrecognized units

### Fixed

- store parsed source files as relative paths

### Documentation

- add change spec for making the repo non-nullable in evals


[0.5.0] - 2026-02-26
--------------------

### Added

- initial import of GNU Units database
- improve parser EOF message
- format output unit expressions more clearly
- better default decimal display
- show build metadata in About widget

### Changed

- use themeMode setting instead of darkMode

### Fixed

- two blank spaces between changelog sections
- raw string lint
- update web page title

### Documentation

- move Phase 8 (complete unit database) up to be 5 instead


[0.4.0] - 2026-02-16
--------------------

Phase 4: Basic UI - Freeform Mode.

- Freeform expression evaluation screen with two-field input (expression +
    optional conversion target) and real-time result display
- Result display widget with four states: idle, success, conversion, and error
- Drawer-based navigation with Freeform and Settings destinations
- Settings screen: precision (2-10), notation
    (decimal/scientific/engineering), dark mode (system/dark/light), evaluation
    mode (real-time/on-submit)
- Riverpod state management with debounced evaluation (500ms, configurable)
- SharedPreferences persistence for user settings
- Quantity formatter supporting decimal, scientific, and engineering notation
- Dimensionless unit stripping in conversion factor computation
- Reciprocal display for unit conversions
- Web platform support
- 848 passing tests


[0.3.0] - 2026-02-16
--------------------

Phase 3: Advanced Unit Features.

- All 7 SI base unit primitives (m, kg, s, K, A, mol, cd)
- Temperature units with affine conversions (tempF, tempC, tempK, tempR) and
  linear degree units (degF, degC, degK, degR)
- Affine unit syntax: function-call form required (e.g., `tempF(212)`)
- 12 SI derived compound units (N, Pa, J, W, Hz, C, V, ohm, F, Wb, T, H)
- 10 physical and mathematical constants (pi, euler, tau, c, gravity, h, etc.)
- Dimensionless primitive units (radian, steradian) with conformability stripping
- SI prefix system: 24 prefixes from quecto to quetta, with prefix-aware unit
  lookup (longest-match-first, separate storage to avoid collisions)
- Refactored Unit hierarchy: Unit subclasses (`PrimitiveUnit`, `CompoundUnit`,
  `AffineUnit`, `PrefixUnit`) replace separate `UnitDefinition` classes
- Standalone `resolveUnit` helper replaces `UnitDefinition.toQuantity`
- Lexer recognizes additional operator characters (Unicode multiply/divide)
- 703 passing tests


[0.2.0] - 2026-02-11
--------------------

Phase 2: Unit System Foundation.

- Dimension, UnitDefinition, Quantity, and Prefix domain classes
- Unit repository with builtin definitions for length, mass, and time
- Unit conversion support integrated into the expression evaluator
- Dimension.dimensionless and Quantity.unity constants
- Chained unit definitions for cleaner builtin unit specs


[0.1.0] - 2026-02-01
------------------

Phase 1: Core Domain.

- Expression lexer and parser with full operator precedence
- Expression evaluator with dimensional analysis
- Support for implicit multiplication
- Mathematical operator support (+, -, *, /, ^, unary negation)
- Parenthesized sub-expressions
- Numeric literals with decimal point support
- 373 passing tests

[Unreleased]: https://github.com/wisnij/unitary/compare/v0.9.8...HEAD
[0.9.8]: https://github.com/wisnij/unitary/compare/v0.9.7...v0.9.8
[0.9.7]: https://github.com/wisnij/unitary/compare/v0.9.6...v0.9.7
[0.9.6]: https://github.com/wisnij/unitary/compare/v0.9.5...v0.9.6
[0.9.5]: https://github.com/wisnij/unitary/compare/v0.9.4...v0.9.5
[0.9.4]: https://github.com/wisnij/unitary/compare/v0.9.3...v0.9.4
[0.9.3]: https://github.com/wisnij/unitary/compare/v0.9.2...v0.9.3
[0.9.2]: https://github.com/wisnij/unitary/compare/v0.9.1...v0.9.2
[0.9.1]: https://github.com/wisnij/unitary/compare/v0.9.0...v0.9.1
[0.9.0]: https://github.com/wisnij/unitary/compare/v0.8.3...v0.9.0
[0.8.3]: https://github.com/wisnij/unitary/compare/v0.8.2...v0.8.3
[0.8.2]: https://github.com/wisnij/unitary/compare/v0.8.1...v0.8.2
[0.8.1]: https://github.com/wisnij/unitary/compare/v0.8.0...v0.8.1
[0.8.0]: https://github.com/wisnij/unitary/compare/v0.7.6...v0.8.0
[0.7.6]: https://github.com/wisnij/unitary/compare/v0.7.5...v0.7.6
[0.7.5]: https://github.com/wisnij/unitary/compare/v0.7.4...v0.7.5
[0.7.4]: https://github.com/wisnij/unitary/compare/v0.7.3...v0.7.4
[0.7.3]: https://github.com/wisnij/unitary/compare/v0.7.2...v0.7.3
[0.7.2]: https://github.com/wisnij/unitary/compare/v0.7.1...v0.7.2
[0.7.1]: https://github.com/wisnij/unitary/compare/v0.7.0...v0.7.1
[0.7.0]: https://github.com/wisnij/unitary/compare/v0.6.10...v0.7.0
[0.6.10]: https://github.com/wisnij/unitary/compare/v0.6.9...v0.6.10
[0.6.9]: https://github.com/wisnij/unitary/compare/v0.6.8...v0.6.9
[0.6.8]: https://github.com/wisnij/unitary/compare/v0.6.7...v0.6.8
[0.6.7]: https://github.com/wisnij/unitary/compare/v0.6.6...v0.6.7
[0.6.6]: https://github.com/wisnij/unitary/compare/v0.6.5...v0.6.6
[0.6.5]: https://github.com/wisnij/unitary/compare/v0.6.4...v0.6.5
[0.6.4]: https://github.com/wisnij/unitary/compare/v0.6.3...v0.6.4
[0.6.3]: https://github.com/wisnij/unitary/compare/v0.6.2...v0.6.3
[0.6.2]: https://github.com/wisnij/unitary/compare/v0.6.1...v0.6.2
[0.6.1]: https://github.com/wisnij/unitary/compare/v0.6.0...v0.6.1
[0.6.0]: https://github.com/wisnij/unitary/compare/v0.5.12...v0.6.0
[0.5.12]: https://github.com/wisnij/unitary/compare/v0.5.11...v0.5.12
[0.5.11]: https://github.com/wisnij/unitary/compare/v0.5.10...v0.5.11
[0.5.10]: https://github.com/wisnij/unitary/compare/v0.5.9...v0.5.10
[0.5.9]: https://github.com/wisnij/unitary/compare/v0.5.8...v0.5.9
[0.5.8]: https://github.com/wisnij/unitary/compare/v0.5.7...v0.5.8
[0.5.7]: https://github.com/wisnij/unitary/compare/v0.5.6...v0.5.7
[0.5.6]: https://github.com/wisnij/unitary/compare/v0.5.5...v0.5.6
[0.5.5]: https://github.com/wisnij/unitary/compare/v0.5.4...v0.5.5
[0.5.4]: https://github.com/wisnij/unitary/compare/v0.5.3...v0.5.4
[0.5.3]: https://github.com/wisnij/unitary/compare/v0.5.2...v0.5.3
[0.5.2]: https://github.com/wisnij/unitary/compare/v0.5.1...v0.5.2
[0.5.1]: https://github.com/wisnij/unitary/compare/v0.5.0...v0.5.1
[0.5.0]: https://github.com/wisnij/unitary/compare/v0.4.0...v0.5.0
[0.4.0]: https://github.com/wisnij/unitary/compare/v0.3.0...v0.4.0
[0.3.0]: https://github.com/wisnij/unitary/compare/v0.2.0...v0.3.0
[0.2.0]: https://github.com/wisnij/unitary/compare/v0.1.0...v0.2.0
[0.1.0]: https://github.com/wisnij/unitary/releases/tag/v0.1.0
