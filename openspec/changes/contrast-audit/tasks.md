...
- [x] 1.1 Create `test/shared/color_contrast_test.dart` with WCAG
  relative-luminance / contrast-ratio helpers and an alpha-compositing helper
  (`fg` over opaque `bg`)
- [x] 1.2 Add threshold assertions for all passing pairings from the audit
  table (muted text, banner text, primary/error text, browse group headers,
  outline border) in both light and dark schemes
- [x] 1.3 Add threshold assertions for the post-fix pairings (source-row
...
- [x] 1.4 Annotate each entry with the widget file/line it mirrors and a
  comment pointing at the `color-contrast` spec's decorative-exemption list

## 2. Worksheet source-row indicator

- [x] 2.1 Write/update widget tests: source row's field shows a
...
- [x] 2.2 Implement the border cue in `worksheet_screen.dart` (per-row
  `enabledBorder`/`border` override when `isActive`), keeping the existing
  `primaryContainer`@0.3 tint as supplementary
- [x] 2.3 Verify no existing worksheet tests assert the old fill-only styling;
  update any that do

## 3. Fast scroll bar colors

- [x] 3.1 Write/update widget tests for the new colors: thumb `primary`@0.8,
  grip lines `onPrimary`@0.9, neighbour labels `onPrimary`@0.85
- [x] 3.2 Update `fast_scroll_bar.dart`: thumb alpha 0.6 → 0.8, grip color
